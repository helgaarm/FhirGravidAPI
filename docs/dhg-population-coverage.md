...
- GET Observation search krever `patient={logical-id}` og aksepterer valgfritt ett `code={system}|{code}` token. POST `_search` bruker `patient.identifier` i form body og krever HelseID utenfor lokal `DevelopmentTestMode`.
...
|---|---|---|
| LOINC | `http://loinc.org` | profile-required eller HL7-recommended coding; norsk tilleggskode publiseres når den er entydig |
...
- body height: norsk SNOMED CT `1153637007` og profile-required LOINC `8302-2`
- body weight: SNOMED CT `27113001` og profile-required LOINC `29463-7`; pre-pregnancy context beholdes som annotation uten å hevde self-report
...
| body height | `http://snomed.info/sct|1153637007` eller `http://loinc.org|8302-2` |
...
Blood pressure components bruker norske SNOMED CT-koder `4471000202106` og `4481000202108` sammen med profile-required LOINC `8480-6` og `8462-4`. Panelkoden forblir LOINC `85354-9` fordi den norske profilen ikke oppgir en entydig SNOMED CT panelkode.
