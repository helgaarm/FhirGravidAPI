...
| `rhesusDNegative.consentFetalRhesusTyping` | UNSUPPORTED | —<br>**Regel:** samtykke konverteres ikke til en clinical Observation; en eventuell FHIR `Consent`-surface krever en eksplisitt architecture- og policybeslutning |
| `rhesusDNegative.fetusRhDPositiveAtWeek24`, `dateForResult` | PARTIAL | presis aggregate DHG-term i `Observation.code.text`; kodeverk 8340 `T002`/`T008` result; resultatdato som text-only component med day precision<br>**Regel:** `true` betyr at minst ett foster er RhD-positivt, `false` betyr at alle fostre er RhD-negative, og `null` utelates; resultatet bindes ikke til ett bestemt foster, og resultatdato utledes ikke som specimen-, effective- eller issued-tid |
| `rhesusDNegative.prophylaxisAtWeek28` | DIRECT | SNOMED CT `408783007`, `valueBoolean`<br>**Regel:** antenatal anti-D prophylaxis status |
| `rhesusDNegative.note` | PARTIAL | text-only `Observation.code`, uparset `valueString`<br>**Regel:** source text beholdes uten tolkning som RhD-resultat, GBS-resultat, diagnose, behandling eller vurdering |
...
| `antenatalAppointments[].appointmentDate` | DIRECT | `Encounter.period` og Observation `effectiveDateTime` med day precision når dato finnes<br>**Regel:** appointment beholdes også uten dato; da utelates `period` og `effective[x]`, mens Encounter status forblir `unknown` |
...
| gestational week/day | DIRECT | LOINC `18185-9`, UCUM `d`<br>**Regel:** ett exact total-day Quantity per appointment; original `week+day` beholdes som annotation, og `effective[x]` utelates når appointment date mangler |
| mother weight | DIRECT | SNOMED CT `27113001` + LOINC `29463-7`, UCUM `kg`<br>**Regel:** norsk SNOMED CT coding og HL7 interoperability coding; refererer alltid til source-Encounter, mens `effective[x]` bare settes når appointment date finnes |
...
| `fetusesVitalSigns[].fosterId` | DIRECT | minimal pregnancy-scoped `Patient.id`; referert fra `Observation.focus`<br>**Regel:** bare positivt, eksplisitt `fosterId`; ID-en avledes deterministisk fra maternal logical ID, aktivt DHG `recordId` og `fosterId`, inneholder ikke NIN eller raw source identifiers, og brukes ikke som clinical identifier |
| `fetusesVitalSigns[].fetalHeartRate` | DIRECT | SNOMED CT `364075005` + LOINC `55283-6`, UCUM `{beats}/min`, `valueQuantity`<br>**Regel:** bare positiv source value; mor er `subject`, og en fetus Patient er `focus` bare når positivt `fosterId` finnes |
...
| fetus entry uten positivt `fosterId` | PARTIAL | fetus Observation med mor som `subject`, uten `focus` eller fetus Patient<br>**Regel:** eksplisitte funn beholdes, men kan ikke korreleres til et bestemt foster på tvers av appointments; begrensningen følger i `Observation.note` |
| appointment uten `appointmentDate` | PARTIAL | Encounter uten `period`; encounter-scoped Observations uten `effective[x]`<br>**Regel:** source-fakta og grouping beholdes uten å konstruere temporal context |
...
- Numeric measurements med en dokumentert DHG positivity constraint utelates når value er `0` eller negativ. Pre-pregnancy height, weight og BMI, `numberOfFetuses`, fetal heart rate og pregnancy week må være positive, days after full week må være `0..6`, og blood pressure components må være positive. Bare et positivt `fosterId` etablerer fetus-identitet; manglende eller ikke-positiv verdi hindrer ikke source-preserving funn uten `focus`. Dette er source-contract validation, ikke clinical reference-range inference.
