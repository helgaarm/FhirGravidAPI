...
- Hvert appointment med gyldig gestational week/day beholder én gestational-age Observation med LOINC `18185-9` og exact total days som UCUM `d`. Når appointment date finnes, kan consumer velge nyeste `effectiveDateTime`; uten dato utelates `effective[x]`, og fasaden lager verken tid eller en duplicate «latest» fact.
- `rhesusDNegative.fetusRhDPositiveAtWeek24` eksponeres som et aggregert positivt/negativt laboratory result for aktuelt svangerskap; det bindes ikke til ett foster. `dateForResult` beholdes som day-precision component og note som separat uparset tekst. Consent eksponeres ikke som Observation, mens antenatal anti-D prophylaxis status fortsatt mappes.
...
- Positivt `fosterId` gir en minimal pregnancy-scoped fetus Patient. Fetal heart rate, presentation/lie, maternal report of movements og uparset note bruker mor som `Observation.subject`, source-appointment som `encounter` og fetus Patient som `focus` når positiv identitet finnes. Uten positivt `fosterId` beholdes funnet uten `focus`; uten appointment date utelates Encounter `period` og Observation `effective[x]`. Edema grade beholdes som raw encounter-scoped integer `0..3`; fasaden navngir eller tolker ikke scale-trinnene.
...
- `pointsOfContact.generalPractitioner`, `midwife`, `maternityHealthcareCentre` og `birthInstitute` eksponeres i patient-scoped `CareTeam`. Fastlege og jordmor bruker contained Practitioner + Organization + PractitionerRole, mens helsestasjon og fødeinstitusjon er direkte contained Organization-participants med source-preserving type. Source-provided HPR number eksponeres for fastlege og jordmor; fastlegens source-provided organisasjonsnummer eksponeres med ENH identifier system. For helsestasjon og fødeinstitusjon konstrueres ingen identifiers, managing responsibility eller directory-data.
