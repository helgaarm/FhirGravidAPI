...
- HBV surface antigen har tilstrekkelig source precision for en SNOMED-coded positive/negative Observation. DHG definerer også HIV, syphilis, Chlamydia, toxoplasmosis og hepatitis C som broad test results der `true` er positivt, `false` negativt og `null` ikke tatt; disse eksponeres med presis DHG-term i `Observation.code.text` og kodeverk 8340 som result, uten å konstruere assay-specific eller facade codes. `hbvCore`, Rubella og blood-antibody-felt holdes tilbake fordi contract terminology er motstridende.
