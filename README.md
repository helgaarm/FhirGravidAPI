...
Når DHG leverer et positivt `fetusesVitalSigns[].fosterId`, opprettes en separat minimal fetus `Patient`. Fetus-spesifikke Observations beholder mor som `subject` og refererer til fosteret med `focus`. Uten positivt `fosterId` beholdes selve funnet med mor som `subject`, men uten `focus` eller konstruert fetus-identitet. Fetus Patient har ingen NIN, name, gender, birthDate eller identifier; den kan leses med samme maternal `X-Patient-Context` via `GET /fhir/Patient/{fetus-id}`. Manglende appointment date gir Encounter uten `period` og Observations uten `effective[x]`; fasaden konstruerer ikke tid. Se [DHG→FHIR-ressursmapping](docs/dhg-fhir-resource-mapping.md).
